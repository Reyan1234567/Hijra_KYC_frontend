 ...
 * - Edit and view actions for draft forms
 * - "Send to HO" functionality to submit drafts (changes status from 0 to 1)
 ...
 * - "Send to HO" button with confirmation dialog
 * - Modal interactions:
...
const DraftsMakeFormTable = () => {
  ...
  const { data, isLoading, isError, isSuccess, error } = useQuery({
    ...
    queryFn: () =>
      getDraftedMakes(date, USER?.user?.userId, pageSize, pageNumber, search),
  });
  ...
  const sendToHoMutation = useMutation({
    ...
    onError: (error: any) => {
      messageApi.error(
        `Error sending to HO: ${error.response?.data?.message || error.message}`
      );
    },
  });
